...
##*This document was written in R MarkDown and regularly updated to GitHub using the Commit and Push commands in Rserver.* 
...
covid_data %>% 
  ...
  #select and created a new table for area_name = "Brentwood", so area name column no longer needed and is dropped
...
brentwood_complete_covid_data %>%
#create a new column day_to_match, don't need to use lubricate because already in date format (not a character)
dplyr::mutate(
...
#use data from this table
brentwood_complete_covid_data %>%
#join the new table brentwood_day_before into the originbal table brentwood_complete_covid_date
dplyr::left_join(
  brentwood_day_before,
...
essex_covid_pop <-
 pop_covid_data %>%
 dplyr::filter(
   area_name %in% c("Brentwood", "Harlow", "Epping Forest", "Chelmsford", "Basildon", "Southend-on-sea", "Braintree", "Colchester")
   ) %>%
 dplyr:: mutate(
   percent_pop_infected = ((cumCasesBySpecimenDate / area_population)* 100)
 ) %>%
 dplyr::group_by(specimen_date) %>%
 dplyr::arrange(area_name)
```

###Brentwood has a population of 77032  ands is situated in the county of Essex.  Essex contains 10 major settlements which are in the covid-19 dataset: Harlow, Epping Forest, Brentwood, Chelmsford, Basildon, Southend-on-sea, Braintree and Colchester.
